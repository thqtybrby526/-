...
export default function Home() {
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col font-sans" dir="rtl" id="homepage-root">
      ...
      <section className="bg-white border-b border-slate-200 py-5 sm:py-8 px-3 md:px-8" id="live-metrics-banner">
        <div className="max-w-7xl mx-auto grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 md:gap-4">
          
          {/* Item 1 */}
          <div className="bg-slate-50 p-3.5 sm:p-5 rounded-2xl border border-slate-150 flex items-center gap-3 sm:gap-4 hover:border-indigo-650/30 transition shadow-xs element-reveal" id="metric-card-1">
            ...
            <div>
              <span className="text-[10px] sm:text-[11px] text-slate-400 block font-black">الدليل الأكاديمي</span>
              <strong className="text-xs sm:text-sm font-extrabold text-slate-900 font-sans block leading-tight">
                ١٧ تخصصاً وشعبة معتمدة
              </strong>
              <p className="text-[9px] sm:text-[10px] text-slate-500 font-medium font-sans">مغطي ببيانات تفصيلية حية بالمرجع</p>
            </div>
          </div>

          {/* Item 2 */}
          <div className="bg-slate-50 p-3.5 sm:p-5 rounded-2xl border border-slate-150 flex items-center gap-3 sm:gap-4 hover:border-emerald-650/30 transition shadow-xs element-reveal" id="metric-card-2">
            ...
            <div>
              <span className="text-[10px] sm:text-[11px] text-slate-400 block font-black">النزاهة الأكاديمية</span>
              <strong className="text-xs sm:text-sm font-extrabold text-slate-900 font-sans block leading-tight">معايير صدق ١٠٠٪</strong>
              <p className="text-[9px] sm:text-[10px] text-emerald-600 font-bold font-sans">توجيه صريح ضد مبالغات التوظيف المباشر</p>
            </div>
          </div>

          {/* Item 3 */}
          <div className="bg-slate-50 p-3.5 sm:p-5 rounded-2xl border border-slate-150 flex items-center gap-3 sm:gap-4 hover:border-amber-650/30 transition shadow-xs element-reveal" id="metric-card-3">
            <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-xl bg-amber-50 text-[#FF7F50] flex items-center justify-center shrink-0">
              <Ticket className="w-5 h-5 sm:w-6 sm:h-6" />
            </div>
            <div>
              <span className="text-[10px] sm:text-[11px] text-slate-400 block font-black">خصومات المصروفات</span>
              <strong className="text-xs sm:text-sm font-extrabold text-[#FF7F50] font-sans block leading-tight">حفظ خصومات الدفعة</strong>
              <p className="text-[9px] sm:text-[10px] text-slate-500 font-medium font-sans">حجز المقاعد مجاني ومثبت للعام الجديد</p>
            </div>
          </div>

          {/* Item 4 */}
          <div className="bg-slate-50 p-3.5 sm:p-5 rounded-2xl border border-slate-150 flex items-center gap-3 sm:gap-4 hover:border-blue-650/30 transition shadow-xs element-reveal" id="metric-card-4">
            <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-xl bg-[#0A2463]/5 text-[#0A2463] flex items-center justify-center shrink-0">
              <Award className="w-5 h-5 sm:w-6 sm:h-6" />
            </div>
            <div>
              <span className="text-[10px] sm:text-[11px] text-slate-400 block font-black">الاعتمادات والتأجيل</span>
              <strong className="text-xs sm:text-sm font-extrabold text-slate-900 font-sans block leading-tight">شهادات مهنية معتمدة</strong>
              <p className="text-[9px] sm:text-[10px] text-indigo-600 font-bold font-sans">تأجيل الموقف التجنيدي للذكور متاح</p>
            </div>
          </div>
      ...
      {/* 2.3 Institute Statistics Component (إحصائيات المعاهد والطلاب) */}
      <section className="bg-gradient-to-b from-white to-slate-50 py-6 sm:py-10 px-3 md:px-8 border-b border-slate-200 text-right" id="institute-statistics">
        <div className="max-w-7xl mx-auto space-y-4 sm:space-y-6">
          <div className="text-center md:text-right space-y-1 element-reveal">
            <h3 className="text-[10px] sm:text-xs font-black text-[#FF7F50] uppercase tracking-wider flex items-center gap-1 justify-center md:justify-start">
              <span>📊 الأرقام القياسية للبوابة والمؤسسات الشريكة</span>
            </h3>
            <h2 className="text-lg sm:text-2xl font-black text-[#0A2463]">
              إحصائيات المعاهد المقيدة والمسجلة لعام ٢٠٢٦
            </h2>
            <p className="text-[10px] sm:text-[11px] text-slate-500 font-bold max-w-2xl mx-auto md:mx-0">
              إحصائيات تقريبية مستقاة مباشرة من سجلات الطلاب المسجلين وعقود الشراكة والتوظيف المبرمة مع كبرى الجهات.
            </p>
          </div>

          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
            {/* Stat Card 1 */}
            <div className="bg-white p-3.5 sm:p-5 rounded-2xl sm:rounded-3xl border border-slate-150 hover:border-indigo-600/30 hover:shadow-md transition-all duration-300 flex flex-col justify-between group element-reveal">
              <span className="text-xl sm:text-2xl mb-1 group-hover:scale-110 transition-transform duration-300 w-fit">🏛️</span>
              <div>
                <span className="text-xl sm:text-[32px] font-black text-[#0A2463] font-mono block leading-none mb-1">
                  {instCount}
                </span>
                <span className="text-[11px] sm:text-xs font-extrabold text-slate-800">المعاهد والأكاديميات</span>
                <p className="text-[9px] sm:text-[10px] text-slate-400 font-semibold mt-0.5 leading-tight">معاهد تدريب خاصة مسجلة وموثقة</p>
              </div>
            </div>

            {/* Stat Card 2 */}
            <div className="bg-white p-3.5 sm:p-5 rounded-2xl sm:rounded-3xl border border-slate-150 hover:border-emerald-600/30 hover:shadow-md transition-all duration-300 flex flex-col justify-between group element-reveal">
              <span className="text-xl sm:text-2xl mb-1 group-hover:scale-110 transition-transform duration-300 w-fit">🎓</span>
              <div>
                <span className="text-xl sm:text-[32px] font-black text-emerald-600 font-mono block leading-none mb-1">
                  {studCount.toLocaleString("ar-EG")}
                </span>
                <span className="text-[11px] sm:text-xs font-extrabold text-slate-800">طالب وطالبة</span>
                <p className="text-[9px] sm:text-[10px] text-slate-400 font-semibold mt-0.5 leading-tight">قاموا بالاستعلام والتسجيل واستخراج الاستمارات</p>
              </div>
            </div>

            {/* Stat Card 3 */}
            <div className="bg-white p-3.5 sm:p-5 rounded-2xl sm:rounded-3xl border border-slate-150 hover:border-amber-600/30 hover:shadow-md transition-all duration-300 flex flex-col justify-between group element-reveal">
              <span className="text-xl sm:text-2xl mb-1 group-hover:scale-110 transition-transform duration-300 w-fit">💼</span>
              <div>
                <span className="text-xl sm:text-[32px] font-black text-amber-600 font-mono block leading-none mb-1">
                  {partCount}
                </span>
                <span className="text-[11px] sm:text-xs font-extrabold text-slate-800">بروتوكول توظيف</span>
                <p className="text-[9px] sm:text-[10px] text-slate-400 font-semibold mt-0.5 leading-tight">شراكات مبرمة لتوجيه الطلاب وتوظيفهم</p>
              </div>
            </div>

            {/* Stat Card 4 */}
            <div className="bg-white p-3.5 sm:p-5 rounded-2xl sm:rounded-3xl border border-slate-150 hover:border-rose-600/30 hover:shadow-md transition-all duration-300 flex flex-col justify-between group element-reveal">
              <span className="text-xl sm:text-2xl mb-1 group-hover:scale-110 transition-transform duration-300 w-fit">⭐</span>
              <div>
                <span className="text-xl sm:text-[32px] font-black text-rose-600 font-mono block leading-none mb-1">
                  %{satCount}
                </span>
                <span className="text-[11px] sm:text-xs font-extrabold text-slate-800">نسبة الرضا والنجاح</span>
                <p className="text-[9px] sm:text-[10px] text-slate-400 font-semibold mt-0.5 leading-tight">تقييم الطلاب لتجربة الدعم والاعتماد</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <section className="bg-slate-100 py-6 sm:py-10 px-3 md:px-8 border-b border-slate-200" id="homepage-tabs-section">
        <div className="max-w-7xl mx-auto space-y-4 sm:space-y-6">
          <div className="text-center md:text-right space-y-1 sm:space-y-2 element-reveal">
            <div className="flex items-center gap-2 justify-center md:justify-start">
              <Sparkles className="w-4 h-4 sm:w-5 sm:h-5 text-amber-500 shrink-0" />
              <span className="text-[10px] sm:text-xs font-black text-amber-600 tracking-wider">التفاصيل التنظيمية المعتمدة</span>
            </div>
            <h2 className="text-lg sm:text-2xl font-black text-[#0A2463] font-sans leading-tight">
              الاعتمادات، المصروفات الرسمية، وملفات القبول لدفعة 2026 🏛️
            </h2>
            <p className="text-[11px] sm:text-xs text-slate-500 leading-relaxed font-semibold max-w-3xl">
          ...
          {/* Tab buttons rows */}
          <div className="grid grid-cols-2 md:grid-cols-4 gap-1.5 sm:gap-2 border-b border-slate-205 pb-2 sm:pb-3 font-sans element-reveal" id="home-tabs-bar">
            {[
              ...
              <button
                ...
                className={`py-2 sm:py-3.5 px-2 sm:px-3 rounded-xl border text-[10px] sm:text-xs font-bold leading-tight transition-all duration-150 cursor-pointer flex items-center justify-center gap-1 sm:gap-1.5 shadow-3xs ${
          ...
          <div className="bg-white rounded-2xl sm:rounded-3xl p-3.5 sm:p-6 border border-slate-200 shadow-3xs text-right leading-relaxed element-reveal" id="home-tabs-content">
            
            {/* Tab 1 content */}
            {activeHomeTab === "tab1" && (
              <div className="space-y-4 sm:space-y-6 animate-fade-in" id="content-tab1">
                {/* Warning Banner */}
                <div className="bg-red-50 border-r-4 border-red-500 p-3 sm:p-4 rounded-r-xl rounded-l-lg text-red-900 text-[11px] sm:text-sm font-black flex items-start gap-1.5 sm:gap-2 leading-relaxed">
                ...
                <div className="space-y-2.5 sm:space-y-3.5">
                  <div className="flex items-start gap-2.5 sm:gap-3 bg-slate-50 p-3 sm:p-4 rounded-xl border border-slate-150 text-[11px] sm:text-xs text-slate-705 leading-relaxed font-semibold">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 pt-1 sm:pt-2">
                  <div className="p-3 sm:p-4 bg-slate-50 border border-slate-150 rounded-2xl hover:border-amber-500/30 transition shadow-3xs text-right">
                    <span className="text-lg sm:text-xl block mb-1">🦅</span>
                    <h4 className="font-extrabold text-[11px] sm:text-sm text-[#0A2463] mb-0.5">اعتماد جامعة حكومية + ختم النسر</h4>
                    <p className="text-[10px] sm:text-[11px] text-slate-500 leading-normal">معتمد مع ختم النسر الرسمي والتوثيق القانوني للملفات.</p>
                  </div>
                  <div className="p-3 sm:p-4 bg-slate-50 border border-slate-150 rounded-2xl hover:border-amber-500/30 transition shadow-3xs text-right">
                    <span className="text-lg sm:text-xl block mb-1">🌍</span>
      ...
      <section className="bg-slate-50 py-6 sm:py-10 px-3 md:px-8 border-b border-slate-205" id="home-interactive-suite">
        <div className="max-w-7xl mx-auto space-y-6 sm:space-y-10">
          
          {/* Centered Ticket & Cohesion Header */}
          <div className="text-center space-y-3 sm:space-y-4 max-w-3xl mx-auto element-reveal">
            <span className="text-[10px] sm:text-xs font-black text-emerald-600 bg-emerald-50 border border-emerald-100 px-2.5 py-1.5 rounded-full uppercase tracking-wider">
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto p-3 md:p-8 grid grid-cols-1 lg:grid-cols-12 gap-4 sm:gap-8 my-1 sm:my-2" id="home-split-container">
        ...
        <div className="lg:col-span-8 space-y-5 sm:space-y-8 text-right" id="home-left-rail">
          ...
          <div className="space-y-3 sm:space-y-4" id="news-section-feed">
            <div className="flex items-center gap-2 border-b border-slate-200 pb-2 sm:pb-3 justify-start">
              <div className="w-6 h-6 sm:w-7 sm:h-7 rounded-lg bg-indigo-500/10 text-indigo-600 flex items-center justify-center">
                <BookMarked className="w-3.5 h-3.5 sm:w-4 sm:h-4" />
              </div>
              <h2 className="text-base sm:text-xl font-black text-amber-400 font-sans shadow-sm">
            ...
            {isLoadingPosts ? (
              <div className="space-y-3 sm:space-y-4 font-sans" id="news-skeletons">
                {[1, 2, 3].map((num) => (
                  <div key={num} className="p-3.5 sm:p-6 bg-white rounded-2xl border border-slate-200 animate-pulse space-y-2.5 sm:space-y-3">
              ...
              <div className="grid grid-cols-1 gap-3 sm:gap-4" id="news-posts-grid">
                {newsPosts.length === 0 ? (
                  <div className="p-6 sm:p-8 text-center text-slate-400 font-bold bg-white border border-slate-150 rounded-2xl text-xs sm:text-sm">
                  ...
                  newsPosts.map((art) => {
                    ...
                    return (
                      <div 
                        key={art.id}
                        className="p-3.5 sm:p-6 bg-white rounded-2xl border border-slate-200 hover:border-amber-500/20 hover:shadow-xs transition duration-200 space-y-2.5 sm:space-y-3 relative overflow-hidden"
                      >
                        <div className="flex flex-wrap items-center justify-between gap-1.5 sm:gap-2.5">
                          <span className="bg-indigo-50 text-[#0A2463] px-2 sm:px-2.5 py-0.5 rounded-md text-[9px] sm:text-[10px] font-black border border-indigo-100">
                            {art.category}
                          </span>
        ...
        <div className="lg:col-span-4 space-y-4 sm:space-y-6 text-right" id="home-right-rail">
          ...
          <div className="bg-gradient-to-br from-[#0A2463] to-slate-900 text-white p-4 sm:p-6 rounded-3xl border border-slate-800 space-y-3 sm:space-y-4 shadow-sm" id="home-sidebar-guidance-card">
            <div className="flex items-center gap-2.5 text-amber-400">
              <div className="w-8 h-8 rounded-full bg-amber-500/10 flex items-center justify-center border border-amber-500/30 chat-bot-pulse shrink-0">
          ...
          <div className="bg-white p-4 sm:p-6 rounded-3xl border border-slate-200 space-y-3 sm:space-y-4" id="home-depts-mini-list">
            <h3 className="font-extrabold text-sm sm:text-md text-[#0A2463] border-b border-slate-100 pb-2">
            ...
            <div className="grid grid-cols-1 gap-1.5 sm:gap-2">
              {ACADEMY_DEPARTMENTS.slice(0, 7).map((dept) => (
                <Link
                  ...
                  className="w-full text-right p-2 hover:bg-slate-50 text-[11px] sm:text-xs font-semibold text-slate-700 rounded-lg transition border border-slate-100 hover:border-slate-300 flex items-center justify-between cursor-pointer font-sans"
                >
                  <span>{dept.name}</span>
            ...
            <Link 
              to="/departments"
              className="text-[11px] sm:text-xs text-[#0A2463] hover:text-amber-500 font-extrabold text-center block pt-1 sm:pt-2 underline underline-offset-4"
            >
