...
const drawFormCanvas = (
  ...
  // Upscale to double resolution (2400x3400) for highly crisp, professional print quality
  canvas.width = 2400;
  canvas.height = 3400;
  const ctx = canvas.getContext("2d");
  if (!ctx) return canvas;

  // Scale context to draw coordinates at 1200x1700
  ctx.scale(2, 2);
...
export default function FormExtraction() {
  ...
  // Load available fields of studies
  useEffect(() => {
    const list = getDynamicDepartments();
    setAvailableDepts(list);
    ...
    fetch("/api/roi-departments")
      ...
      .catch(err => {
        console.warn("Fallback to premium local departments:", err);
      });
  }, []);

  // Safe local offline QR Code generator
  useEffect(() => {
    if (!lead?.reservationCode) {
      setQrBlobUrl("");
      return;
    }
    const targetUrl = `${window.location.origin}/form-extraction?ref=${encodeURIComponent(lead.reservationCode)}`;
    
    QRCode.toDataURL(targetUrl, {
      width: 180,
      margin: 1.5,
      color: {
        dark: "#0a2463", // matches exactly our deep academic dark blue
        light: "#ffffff",
      },
    })
    .then((base64Url) => {
  ...
  // Auto-render form to real image file (Base64) when step becomes 4
  useEffect(() => {
    if (step === 4 && lead) {
      ...
      const renderTimer = setTimeout(async () => {
        try {
          const qrImg = qrBlobUrl ? await loadQRImage(qrBlobUrl) : null;
          let logoImg: HTMLImageElement | null = null;
          try {
            logoImg = await loadQRImage(AcademicLogo);
          } catch (e) {
            console.warn("Could not load academic logo for canvas drawing: ", e);
          }
          ...
          console.error("Failed to programmatically draw form canvas:", err);
        } finally {
  ...
  }, [step, lead?.reservationCode, academyName, selectedDept, agentName, qrBlobUrl]);
  ...
  const handleTackleFormGeneration = async (e: React.FormEvent) => {
    ...
    // Save adjustments to DB server beforehand to assure sync
    await handleSaveDetails();
    
    setStep(4);
    toast.success("✨ تم صياغة وتوليد وطباعة وثيقة استمارتكم المعتمدة بنجاح!");
  };
  ...
  // 3. Print Form Directly (Using secure, isolated print iframe)
  const handlePrint = async () => {
    ...
    try {
      ...
      // Create a hidden, sandboxed isolated print iframe
