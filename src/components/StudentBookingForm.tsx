    ...
    <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden shadow-xs" id="booking-form-wrapper" dir="rtl">
      ...
      {!successResult ? (
        isConfirming ? (
          ...
          <div className="flex items-center justify-between gap-2 text-[11px] font-bold bg-slate-50 border border-slate-100 rounded-xl py-2 px-3.5 select-none text-right">
            <div className="flex items-center gap-2">
              <span className={`w-2 h-2 rounded-full ${isOnline ? "bg-emerald-500 animate-pulse" : "bg-rose-500 animate-ping"}`} />
              <span className={isOnline ? "text-slate-600" : "text-rose-650"}>
                {isOnline ? "حالة الاتصال المركزي بالمنصة: متصل وآمن" : "انتبه: غير متصل بالإنترنت! قد تفشل كتابة وتأكيد الحجز المبدئي"}
              </span>
            </div>
            <span className="text-slate-400 font-medium">بوابة القبول v1.0</span>
          </div>
