...
export function getDynamicDepartments(): { id: string; name: string }[] {
  ...
  const filteredDynamicList = list.filter(name => {
    const norm = name.toLowerCase();
    return !invalidKeywords.some(kw => norm.includes(kw)) && name.length < 80;
  });

  const mergedList = [...filteredDynamicList, ...REAL_ACADEMY_DEPARTMENTS];
...
function ChatAdvisorThinkingIndicator() {
  const [step, setStep] = useState(0);
  const steps = [
    "جاري تحليل استفسارك وبحث الملفات الأكاديمية... 🧠",
    "جاري مقارنة التخصصات واختيار التوجيه المهني الأنسب... 🔍",
    "المستشار الأكاديمي يقوم بصياغة التوجيه الذكي لك الآن... ✍️"
  ];
  ...
  return (
    <div className="flex justify-start animate-pulse">
      <div className="bg-amber-50/80 border border-amber-200/60 rounded-2xl p-3 max-w-[85%] text-[11px] font-bold text-amber-900 flex flex-col gap-1.5 shadow-3xs text-right" dir="rtl">
        <div className="flex items-center gap-2">
          ...
          <span className="text-slate-500 font-medium text-[10px]">المستشار الذكي يفكر</span>
        </div>
        <p className="text-[10px] text-slate-700 font-semibold">{steps[step]}</p>
      </div>
    </div>
  );
}
...
function TypewriterMessage({ text, onComplete }: { text: string; onComplete?: () => void }) {
  ...
  useEffect(() => {
    ...
    const typeWord = () => {
      if (index < words.length) {
        ...
        index++;
        timer = setTimeout(typeWord, 30 + Math.random() * 25);
      } else {
  ...
  return (
    <p className="whitespace-pre-line">
      {displayedText}
      {!isTypingComplete && <span className="inline-block w-1.5 h-3.5 bg-indigo-600 animate-pulse ml-1 align-middle" />}
    </p>
  );
}
    ...
    <>
      ...
      <div className="fixed top-1/2 -translate-y-1/2 right-2 sm:right-6 z-[9995] cursor-pointer" onClick={() => setIsOpen(!isOpen)}>
        <div className="relative">
          ...
          <button className="bg-gradient-to-r from-[#0A2463] to-[#112f75] text-white p-3 sm:px-5 sm:py-3 rounded-full shadow-xl flex items-center justify-center gap-2 border border-amber-400/50 chat-bot-pulse">
      ...
      {/* 3. Overlay - خلفية سوداء شفافة تقفل الشات لما تضغط عليها */}
      {isOpen && (
        <div 
      ...
      {/* 4. نافذة الشات */}
      {isOpen && (
        <div className="fixed top-1/2 -translate-y-1/2 right-4 left-4 sm:right-6 sm:left-auto z-[9999] w-auto sm:w-[360px] h-screen bg-white rounded-3xl shadow-2xl border border-slate-200 flex flex-col overflow-hidden" dir="rtl">
          ...
          <div className="flex-1 flex flex-col relative overflow-hidden">
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-3.5 bg-slate-50">
              {messages.map((m, mIdx) => {
                const isLatestBot = m.sender === "bot" && mIdx === messages.length - 1;
                return (
                  <div key={m.id || mIdx} className={`flex ${m.sender === "bot" ? "justify-start" : "justify-end"}`}>
                    <div className={`max-w-[85%] rounded-2xl p-3.5 text-xs font-semibold leading-relaxed ${m.sender === "bot" ? "bg-white text-slate-800 border shadow-xs" : "bg-[#0A2463] text-white"}`}>
                      {m.sender === "bot" && isLatestBot && !m.typed ? (
                        <TypewriterMessage 
                          text={m.text} 
                          onComplete={() => {
                            setMessages(prev => prev.map((msg, idx) => idx === mIdx ? { ...msg, typed: true } : msg));
                          }}
                        />
                      ) : (
              ...
              {isTyping && <ChatAdvisorThinkingIndicator />}
            </div>
            ...
            <div className="p-3.5 border-t bg-white shrink-0">
              <form onSubmit={handleSend} className="flex gap-2">
                ...
                />
                <button type="submit" className="bg-[#0A2463] hover:bg-[#112f75] text-white p-2.5 rounded-xl transition" disabled={!isOnline && inputVal.trim() !== ""}><Send className="w-4 h-4" /></button>
              </form>
            </div>

            <div className="px-3.5 py-1.5 bg-slate-50 border-t flex justify-between items-center text-[9px] font-bold text-slate-400 select-none">
              <div className="flex items-center gap-1.5">
                <span className={`w-1.5 h-1.5 rounded-full ${isOnline ? "bg-emerald-500 animate-pulse" : "bg-rose-500 animate-ping"}`} />
                <span>{isOnline ? "الاتصال بالخادم الموحد آمن ومستقر 🟢" : "أنت غير متصل بالإنترنت! يرجى التحقق من الشبكة ⚠️"}</span>
              </div>
              <span>مستشار القبول الذكي v2.4</span>
            </div>
